...
![ERD ONE](/users/domschrein/desktop/practicum2/Practicum2 PART1ERD.png)
...
for(i in 1:n)
{
  ...
  #a <- xpathSApply(node, ".//History/PubMedPubDate/Year", xmlValue)  
  #b <- xpathSApply(node, ".//History/PubMedPubDate/Month", xmlValue)
  #d <- xpathSApply(node, ".//History/PubMedPubDate/Day", xmlValue)
...
```
```{sql connection=dbcon}
select * from PubmedArticle limit 5;
```
...
Creating Tables
![ERD ONE](/users/domschrein/desktop/practicum2/FactTable.png)
```{sql connection=dbcon}
...
PART 2
Q2.
Creating new tables which ar needed
```{sql connection=dbcon}
DROP TABLE IF EXISTS author;
```
...
![ERD THREE](/users/domschrein/desktop/practicum2/FactTable.png)
